...
- 本仓库可能包含多本小说；默认只聚焦“当前指定的一本”或“正在新建的一本”。
- 禁止读取 `projects/` 下“其他小说”的正文/摘要/设定/索引来作参考、对照或搬运（避免跨书污染与吃书）。
- 当用户未明确指定目标小说时：先问“要操作哪本/是否新建哪本”，不要自行遍历/打开 `projects/*` 来做选择。
- 新建小说时：只使用模板与用户输入推进初始化；同样禁止读取其他小说内容。

提示：进入具体小说工作区（WORKDIR）后，若目录名不是 `novel`，运行脚本时通常需要加 `--novel-dir <本目录名>`（例如 `novel-analytics` / `novel-indexing-and-searching`）。
...
- 若用户未指定目标小说：先询问“要操作哪本/是否新建哪本”，不要自行打开其他小说目录来“找线索”或“对照参考”。
